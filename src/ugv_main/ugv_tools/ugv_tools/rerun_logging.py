...
class RerunLogging(Node):
    def __init__(self):
        ...
        self.rerun_addr = f"{rerun_ip}:{rerun_port}"

        rr.init("rover", spawn=False)
        rr.connect_tcp(self.rerun_addr)

        self.get_logger().info(f"started logging to rerun at {self.rerun_addr}")
    ...
    def move(self):
        ...
        self.frame += 1
        # rr.log("rover", rr.Transform3D(clear=False, translation=(self.frame * 0.1, 0, 0)))
        rr.log("rover/base_footprint/base_link/pt_base_link/pt_link1", rr.Transform3D(clear=False, translation=(self.frame * 0.1, 0, 0)))
        # rr.log(
        #     "rover/base_footprint/base_link/pt_base_link/pt_link",
        #     rr.Transform3D(clear=False, rotation=rr.Quaternion(xyzw=(self.frame / 10, 0, 0, 0)))
        # )
    ...
    def log_image(self, image: msg.Image):
    ...
    def log_lines(self, lines: LineArray):
        ...
        rr.log(
            "camera/image/lines",
            rr.LineStrips2D(strips=line_strips, colors=[(255,0,0)]*len(line_strips))
        )
